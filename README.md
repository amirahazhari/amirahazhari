# Hi, I'm Amirah Zulaikha 👋  
###  🎨 UI/UX Designer | 💻 Front-End Developer |💡 Business Analyst  

I’m passionate about bridging business goals with user-centered design and modern technology.  
Currently exploring how digital transformation and automation can simplify workflows and improve user experiences.

---

### 🌟 What I Do
- **Business Analysis**: System migration, UAT facilitation, requirements gathering, and process automation  
- **UI/UX Design**: Figma, user research, and prototyping  
- **Front-End Development**: HTML, CSS, JavaScript, and React

---

### 🚀 Projects
- 🧭 **MyEnergyStats** — Liferay-based energy dashboard with custom navigation and UI enhancements  
- 🧩 **Automation Toolkit** — Excel macros for data cleaning and system reports  
- 💡 **UX Case Study: EnergyKids Portal** — Improving information accessibility for younger users

---

### 📫 Connect with Me
- 💼 [LinkedIn](https://www.linkedin.com/in/amirah-zulaikha-azhari/)
- 🌐 (https://amirah-azhari-portfolio.vercel.app/)
- ✉️ amirahazharii97@gmail.com
- Roadmap Progress:
  
<a href="https://roadmap.sh"><img src="https://roadmap.sh/card/tall/6931ad2d695568122a15e757?variant=dark&roadmaps=6931af19695568122a1684a1" alt="roadmap.sh"/></a>
---

⭐ *“Good design is where clarity meets empathy.”*
